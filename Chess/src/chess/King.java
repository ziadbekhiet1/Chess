...
public class King extends ChessPiece {
    ...
    public boolean move(Board chessboard, int destRow, int destCol) {
        ...
        if (!dest.getOccupied() || dest.getBW() != this.getColor()) {
            ...
            if (start.getPiece().getMoved() == 0) {
                if (destCol == start.getCol() && (Math.abs(distanceRow) == 2) && start.getBW() == 'b') {
                    if (chessboard.getSpace(0, 0).getPiece() instanceof Rook && chessboard.getSpace(0, 0).getPiece().getMoved() == 0 && chessboard.getSpace(0, 7).getPiece() instanceof Rook && chessboard.getSpace(0, 0).getPiece().getMoved() == 0) {
                        return true;
                    }
                } else if (destCol == start.getCol() && (Math.abs(distanceRow) == 2) && start.getBW() == 'w') {
                    if (chessboard.getSpace(7, 0).getPiece() instanceof Rook && chessboard.getSpace(0, 0).getPiece().getMoved() == 0 && chessboard.getSpace(7, 7).getPiece() instanceof Rook && chessboard.getSpace(0, 0).getPiece().getMoved() == 0) {
                        return true;
                    }
                }
            }
